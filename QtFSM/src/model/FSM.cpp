...
QList<Transition*> FSM::transitions() const
{
...
QList<Event*> FSM::events() const
{
    return m_events;
}

State* FSM::initialState() const
{
...
bool FSM::validate(QString *errorMessage) const
{
    ...
    for (Transition *transition : m_transitions) {
        if (!transition->sourceState() || !transition->targetState()) {
            if (errorMessage) {
                *errorMessage = QString("Transition %1 has invalid source or target").arg(transition->id());
            }
            return false;
        }
    }
